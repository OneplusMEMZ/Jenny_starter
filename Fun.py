            ...
            if type(y) == type(1.0):
                ...
                if type(w) == type(1.0):
                    ...
                    if type(h) == type(1.0):
                        ...
                        PlaceDictory["visible"] = True
                        G_UIElementPlaceArray[uiName][elementName]=PlaceDictory
def DoCanvasRecord(drawCanvas,type,x,y,x2,y2,fillcolor,outlinecolor,fillwidth,dash1=0,dash2=0,newImage=None,text='',textFont = None):
    """画板动作处理函数"""
    if  drawCanvas != None:
        if type == 'line' or type == 'pen'  :
            if  dash1 > 0 :
                drawCanvas.create_line(x, y, x2, y2, fill=fillcolor,dash=(dash1,dash2),width = fillwidth)
            else:
                drawCanvas.create_line(x, y, x2, y2,fill=fillcolor, width = fillwidth)
        elif type == 'arrow':
            if  dash1 > 0 :
                drawCanvas.create_line(x, y, x2, y2, arrow=tkinter.LAST,fill=fillcolor,dash=(dash1,dash2),width = fillwidth)
            else:
                drawCanvas.create_line(x, y, x2, y2,arrow=tkinter.LAST,fill=fillcolor, width = fillwidth)
        elif type == 'rect':
            if  dash1 > 0 :
                drawCanvas.create_rectangle(x, y, x2, y2, fill=fillcolor,outline=outlinecolor,dash=(dash1,dash2),width = fillwidth)
            else:
                drawCanvas.create_rectangle(x, y, x2, y2,fill=fillcolor,outline=outlinecolor, width = fillwidth)
        elif type == 'circle':
            if  dash1 > 0 :
                drawCanvas.create_oval(x, y, x2, y2, fill=fillcolor,outline=outlinecolor,dash=(dash1,dash2),width = fillwidth)
            else:
                drawCanvas.create_oval(x, y, x2, y2,fill=fillcolor,outline=outlinecolor, width = fillwidth)
        elif type == 'star':
            center_x = (x + x2)/2
            center_y = (y + y2)/2
            rx = (x2 - x)/2
            ...
            drawCanvas.create_rectangle(x, y, x2, y2,fill=fillcolor, width = 0) 
        elif type == 'text':
            drawCanvas.create_text(x, y,fill=outlinecolor,text=text,font = textFont,anchor='nw',tag='text')
        elif type == 'image':
            drawCanvas.create_image(x, y,image=newImage,anchor='nw',tag='image')
        ...
        while True:
            ...
            if text.find('Canvas:') >= 0:
                ...
                if drawCanvas != None:
                    ...
                    DoCanvasRecord(drawCanvas,type,x1,y1,x2,y2,fill,outline,width,dashx,dashy,newImage,text,textFont)
                continue
        f.close()  
